...
for index, row in df_linkcost[df_linkcost['target'].str.startswith('kz')].iterrows():
    ...
    for i in range(num_intervals):
        df_linkcost.at[index, 'interval' + str(i) + '_cost'] += park_hours * (parking_rate + 0) #conf.config_data['Price_Params']['zip']['ppmin']*60)
...
# # rewrite nodecost for testing
# td_node_cost = np.zeros((5, num_intervals))
# node_ID = df_nodecost[:5][['node_ID', 'in_link_ID', 'out_link_ID']]

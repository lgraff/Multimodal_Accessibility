...
def od_cnx(G_od, o_coord, d_coord):
    ...
    od_cnx_edges = {}
    inv_nid_map = dict(zip(G_od.nid_map.values(), G_od.nid_map.keys())) 
        
    for i_name in ['org','dst']:
        print(inv_nid_map[i_name])
        #***** I think we shoudl not be using G_super.gcd_dist, but rather a newly created one which includes the ODs and their gcd dists
        W_od = conf.config_data['Supernetwork']['W_od'] * conf.config_data['Conversion_Factors']['meters_in_mile']
        catch = ut.wcz(inv_nid_map[i_name], G_od.gcd_dist, W_od)  # find WCZ
        #print('-----')
        # build od connector edge for each FIXED node in the catchment zone 
        for j in catch:
            if j in G_od.nid_map_fixed.keys():
                j_name = G_od.nid_map[j]  # map back to node name
                ...
                walk_time = (G_od.gcd_dist[inv_nid_map[i_name], j] / 
        ...
        catch_node_names = [G_od.nid_map[c] for c in catch]
        ...
        rem_fixed_modes = set(G_od.flex_pre) - set(catch_fixed_modes)
        if rem_fixed_modes:
            for rm in rem_fixed_modes:
                if ((i_name == 'org' and rm == 'k') | (i_name == 'org' and rm == 'kz') | (i_name == 'dst' and rm == 'zd')):   # exceptions 2/5
                    continue
                # nn calc
                nnID, nnName, nnDist = ut.nn(inv_nid_map[i_name], G_od.gcd_dist, rm, G_od.nid_map) 
                r_name = nnName
        ...
        # PV = personal vehicle 
        for m in G_od.flex_pre:
            if i_name == 'dst' and m == 'pv':  # exception 3
                continue
            nnID, nnName, nnDist = ut.nn(inv_nid_map[i_name], G_od.gcd_dist, m, G_od.nid_map) 
            k_name = nnName
            #print(k_name)
...
# G_super_od = copy.copy(G_super)
# print(G_super_od.coord_matrix.shape)
...
# G_super_od.graph.add_nodes_from([('org', {'pos': tuple(o_coord)}),  
#                                  ('dst', {'pos': tuple(d_coord)})])
...
# #%% build od_cnx
# od_cnx_edges = od_cnx(G_super_od, sc_costs_od, o_coord, d_coord)
# #%%
# inv_nid_map = dict(zip(G_super_od.nid_map.values(), G_super_od.nid_map.keys())) 
# nnID, nnName, nnDist = ut.nn(inv_nid_map['org'], G_super_od.gcd_dist, 't', G_super_od.nid_map) 
